...
class Torrentino {
  ...
  getLatestReleases = async (page: void | number): Promise<Array<Object>> =>
    retry(
      async (bail: (err: Error) => void) => {
        ...
        try {
          const $ = await this._connector.htmlGet('movies', {
            quality: 'hq',
  ...
  getReleaseDetails = async (slug: string) => retry(async (
    bail: (err: Error) => void,
  ) => {
    const noReleaseErrMessage = `Failed to get Torrentino release for movie ${slug}`;
