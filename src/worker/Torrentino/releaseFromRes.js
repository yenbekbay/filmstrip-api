...
const movieInfoFromRes = ($: () => Object) => {
  ...
  const year = parseInt(
    detailsNode.find('tr.clause td:contains("Год") ~ td').text(),
    10,
  );
  ...
  const productionCountries = _.flow(
    _.split(','),
    _.map(_.flow(_.trim, _.capitalize, _.replace('Сша', 'США'))),
  )(detailsNode.find('tr.clause td:contains("Страна") ~ td').text());
  const genres = _.flow(_.split(','), _.map(_.trim))(
  ...
  const creditsForJob = (job: string) =>
    _.flow(
      _.split(','),
      _.map(
        _.flow(_.trim, (name: ?string) => name ? {name, photoUrl: null} : null),
      ),
      _.compact,
...
const torrentsFromRes = ($: () => Object) =>
  $('.main > section > .entity > .list-start > .table-list')
    ...
    .map((el: Object) => {
      ...
      const bundledSubtitles = _.flow(_.split(separatorRegex), _.compact)(
      ...
      return quality !== 'SD' &&
        ...
        seeds > 0 &&
        peers > 0 &&
        _.startsWith('magnet:?', magnetLink)
        ? {
            source: 'Torrentino',
            quality,
            audioTranslationType,
